define(["require", "exports", 'event', 'common', 'discocontext', 'contentcommunicatorimpl', 'kernaussagecommunicatorimpl', 'discussioncommunicatorimpl', 'ratingcommunicatorimpl', 'konsenskistemodel', 'kernaussagemodel', 'contentmodel'], function (require, exports, Events, Common, discoContext, ContentCommunicator, KernaussageCommunicator, DiscussionCommunicatorImpl, RatingCommunicatorImpl, KonsenskisteModel, KernaussageModel, ContentModel) {
    var Main = (function () {
        ...
        Main.prototype.createAndAppendKa = function (kokiId, kaData) {
            ...
            var batch = [
                function (r) {
                    content = _this.createContent(kaData, function () { return r(); }, function (err) { return onError(err); });
                },
                function (r) {
                    post = _this.createPost({ typeId: '6', contentId: content.Id }, function () { return r(); }, function (err) { return onError(err); });
                },
                function (r) {
                    reference = _this.createPostReference({ typeId: '11', referrerId: post.Id, referreeId: kokiId.toString() }, function () { return r(); }, function (err) { return onError(err); });
                },
                kaData.context ?
                    (function (r) { return Common.Callbacks.batch([
                        function (r) {
                            cxtContent.Text = kaData.context;
                            cxtContent.CultureId = '2';
                            discoContext.Content.add(cxtContent);
                            discoContext.saveChanges().then(function () { return r(); }).fail(function (error) { return onError(error); });
                        },
                        function (r) {
                    ...
                    ], r); }) : function (r) { return r(); }
            ];
            Common.Callbacks.batch(batch, function (err) {
                if (err)
                    onError(err);
                else {
        ...
        Main.prototype.query = function (id) {
            ...
            }).fail(function (error) { return onError("JayData request failed"); });
            return out;
        };
        Main.prototype.create = function (kokiData, parentTopicId, then) {
            ...
            Common.Callbacks.batch([
                function (r) {
                    content = _this.createContent(kokiData, function () { return r(); }, function (err) { throw err; });
                },
                function (r) {
                    post = _this.createPost({ typeId: '2', contentId: content.Id }, function () { return r(); }, function (err) { throw err; });
                },
                function (r) {
                    if (parentTopicId) {
                        topicReference = _this.createPostReference({ typeId: '2', referrerId: post.Id, referreeId: parentTopicId.toString() }, function () { return r(); }, function (err) { throw err; });
                    }
                    else
                        r();
                }
        ...
        Main.prototype.createContent = function (content, then, fail) {
            ...
            discoContext.saveChanges().then(function () { return then(parseInt(discoContent.Id)); }).fail(function (error) { return fail(error); });
            return discoContent;
        };
        Main.prototype.createPost = function (props, then, fail) {
            ...
            discoContext.saveChanges().then(function () { return then(); }).fail(function (error) { return fail(error); });
            return discoPost;
        };
