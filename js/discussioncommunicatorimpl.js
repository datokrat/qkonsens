define(["require", "exports", 'event', 'common', 'comment', 'contentcommunicatorimpl', 'ratingcommunicatorimpl', 'discocontext'], function (require, exports, Events, Common, Comment, ContentCommunicatorImpl, RatingCommunicatorImpl, discoContext) {
    var Main = (function () {
        ...
        Main.prototype.appendComment = function (discussableId, comment) {
            var _this = this;
            var onError = function (error) { return _this.commentAppendingError.raise({ discussableId: discussableId, comment: comment, error: error }); };
            var content;
            var post;
            var reference;
            Common.Callbacks.batch([
                function (r) {
                    ...
                    discoContext.saveChanges(function () { return r(); }).fail(onError);
                },
                function (r) {
                ...
                function (r) {
                    ...
                    discoContext.saveChanges().then(function () { return r(); }).fail(onError);
                }
            ], function () {
        ...
        Main.prototype.removeComment = function (args) {
            var _this = this;
            var onError = function (error) { return _this.commentRemovalError.raise({ discussableId: args.discussableId, commentId: args.commentId, error: error }); };
            var data = {
            ...
            Common.Callbacks.batch([
                function (r) {
                    Common.Callbacks.atOnce([
                        function (r) { return discoContext.PostReferences.filter('it.ReferreeId == this.commentId', data)
                            .toArray().then(function (refs) { data.referredTimes = refs.length; r(); }).fail(onError); },
                        function (r) { return discoContext.PostReferences.filter('it.ReferrerId == this.commentId', data)
                            .toArray().then(function (refs) { data.refersTimes = refs.length; r(); }).fail(onError); },
                        function (r) { return discoContext.PostReferences.filter('it.ReferrerId == this.commentId && it.ReferreeId == this.discussableId ' +
                            ...
                            .toArray().then(function (refs) { data.references = refs; r(); }).fail(onError); }
                    ], r);
                },
                function (r) {
