    ...
    var Main = (function () {
        ...
        Main.prototype.processSubmitRatingCommand = function () {
            ...
            kCommunicator.rating.submitRating = function (ratableId, rating, then) {
                ...
                test.assert(function () { return ratableId == 6; });
                test.assert(function () { return rating == 'stronglike'; });
                test.assert(function () { return counter.get('then') == 0; });
                then && then();
                test.assert(function () { return counter.get('then') == 1; });
            };
            var cmd = new Rating.SelectRatingCommand('stronglike', function () { return counter.inc('then'); });
            kController.commandProcessor.processCommand(cmd);
            test.assert(function () { return counter.get('submitRating') == 1; });
            test.assert(function () { return counter.get('then') == 1; });
        };
        Main.prototype.edit = function () {
            ...
            commandProcessor.chain.insertAtBeginning(function (cmd) {
                if (cmd instanceof KElementCommands.OpenEditKElementWindowCommand) {
                    counter.inc('cmd');
                    test.assert(function (v) { return v.val(cmd.model.id()) == 6; });
                    return true;
                }
                return false;
            });
