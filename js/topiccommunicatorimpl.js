    ...
    var Main = (function () {
        ...
        Main.prototype.queryChildren = function (id, out) {
            ...
            var then = function (rawChildren) {
                var children = rawChildren.map(function (raw) { return _this.topicParser.parseTopic(raw); });
        ...
        Main.prototype.queryRootChildren = function (then, fail) {
            ...
            discoContext.Posts.filter(function (it) { return it.PostType.Description.Name == 'Topic' && it.RefersTo.every(this.parentlessFilter); }, { parentlessFilter: parentlessFilter })
                .include('Content')
                .toArray()
                .then(then)
                .fail(fail);
        ...
        Main.prototype.queryNonRootChildren = function (id, then, fail) {
            var childFilter = discoContext.PostReferences.filter(function (it) { return it.ReferenceType.Description.Name == 'Child' && it.ReferreeId == this.Id; }, { Id: id });
            discoContext.Posts.filter(function (it) { return it.PostType.Description.Name == 'Topic' && it.RefersTo.some(this.childFilter); }, { childFilter: childFilter })
                .include('Content')
        ...
        Main.prototype.queryContainedKokis = function (id, out) {
            ...
            var then = function (rawKokis) {
                var kokis = rawKokis.map(function (raw) { return _this.kokiParser.parse(raw); });
                _this.containedKokisReceived.raise({ id: id, kokis: kokis });
                out.items.set(kokis);
                out.queryState().loading(false);
            };
            var fail = function (err) { return out.queryState().error(err); };
            if (!id.root)
