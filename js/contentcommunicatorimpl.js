define(["require", "exports", 'discocontext', 'common', 'event'], function(require, exports, discoContext, Common, Events) {
    var ContentCommunicator = (function () {
        ...
        ContentCommunicator.prototype.updateContext = function (model, callbacks) {
            var contextFilter = discoContext.PostReferences.filter('it.ReferenceType.Description.Name == "Context"' + '&& it.ReferrerId == this.ReferrerId', { ReferrerId: model.postId.toString() }).include('ReferenceType.Description').include('Referree.Content');
            ...
            Common.Callbacks.batch([
                ...
                function (r) {
                    context = Common.Coll.where(contextResult, function (item) {
                        return item.ReferenceType.Description.Name == 'Context';
                    })[0].Referree;
                    if (!context)
